...
// Komponenty ikony oka (bez zmian)
...
function RegisterPage() {
    ...
    const handleGoogleSuccess = async (credentialResponse) => {
        ...
        try {
            ...
            setLoading(false);
            setApiError(error.response?.data?.message || 'Błąd logowania przez Google.');
        }
    };
    
    const handleGoogleError = () => {
        setApiError('Logowanie przez Google nie powiodło się. Spróbuj ponownie.');
    };
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <fieldset style={{ border: '1px solid #eee', padding: '15px', borderRadius: '5px' }}>
                    ...
                    <div style={formRowStyle}>
                        ...
                        {validationErrors.password && <p style={errorTextStyle}>{validationErrors.password}</p>}
                    </div>
                    ...
                    <div style={formRowStyle}>
                        ...
                        {validationErrors.confirmPassword && <p style={errorTextStyle}>{validationErrors.confirmPassword}</p>}
                    </div>
                </fieldset>
                ...
                {userType === 'food_truck_owner' && (
                    <fieldset style={{ border: '1px solid #eee', padding: '15px', borderRadius: '5px' }}>
                        ...
                        <div style={formRowStyle}>
                            ...
                            <input id="nip" type="text" name="nip" value={formData.nip} onChange={handleChange} required={userType === 'food_truck_owner'} style={inputStyle} placeholder="1234567890" />
                            {validationErrors.nip && <p style={errorTextStyle}>{validationErrors.nip}</p>}
                        </div>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '10px', ...formRowStyle }}>
                            <div>
                                ...
                                <input id="postalCode" type="text" value={postalCode} onChange={(e) => { setPostalCode(e.target.value); if (validationErrors.postalCode) setValidationErrors(p => ({...p, postalCode: null})); }} required={userType === 'food_truck_owner'} style={inputStyle} placeholder="00-000" />
                                {validationErrors.postalCode && <p style={errorTextStyle}>{validationErrors.postalCode}</p>}
                            </div>
