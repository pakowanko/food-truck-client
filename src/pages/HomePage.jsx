...
const ALL_CUISINES = [
  "Burgery", "Pizza", "Zapiekanki", "Hot-dogi", "Frytki belgijskie", "Nachos", "Kuchnia polska", "Kuchnia azjatycka", "Kuchnia meksykańska", "Lody", "Gofry", "Churros", "Słodkie wypieki", "Kawa", "Lemoniada", "Napoje bezalkoholowe", "Piwo kraftowe", "Opcje wegetariańskie", "Opcje wegańskie", "Opcje bezglutenowe"
];
...
function HomePage() {
  ...
  const fetchProfiles = async (filters = {}) => {
    ...
    try {
      ...
      // ZABEZPIECZENIE: Upewniamy się, że stan `profiles` to zawsze tablica
      setProfiles(Array.isArray(response.data) ? response.data : []);
    } catch (err) {
      setError(err.message || 'Nie udało się pobrać danych.');
      setProfiles([]); // W razie błędu też ustawiamy pustą tablicę
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      <form onSubmit={handleSearch} style={{ display: 'flex', gap: '15px', marginBottom: '30px', padding: '20px', background: '#f9f9f9', borderRadius: '8px' }}>
        ...
        <input type="text" value={postalCode} onChange={e => setPostalCode(e.target.value)} placeholder="Wpisz miasto lub kod pocztowy..." style={{ padding: '10px' }}/>
      ...
      {!loading && !error && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '20px' }}>
          {/* Zabezpieczenie jest już w stanie, ale dla pewności możemy dodać je i tutaj */}
          {profiles && profiles.length > 0 ? (
            profiles.map(profile => (
              <TruckCard key={profile.profile_id} profile={profile} />
            ))
          ) : (
            <p>Nie znaleziono żadnych food trucków. Zarejestruj się jako pierwszy!</p>
          )}
        </div>
      )}
    </div>
